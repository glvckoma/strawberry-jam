...
// Set default values for important settings if not already set
const setDefaultSettingsIfNeeded = () => {
  // Enable UUID spoofing by default - critical for proper authentication
  if (store.get(STORE_KEY_UUID_SPOOFER) === undefined) {
    store.set(STORE_KEY_UUID_SPOOFER, true);
    log("info", "[Settings] UUID spoofing enabled by default");
  }
  
  // Set server swap to 'en' by default for consistent server handling
  if (store.get('debug.locale') === undefined) {
    store.set('debug.locale', 'en');
    log("info", "[Settings] Server locale set to 'en' by default");
  }
};
...
app.whenReady().then(() => {
  ...
  const showTools = config.showTools;
  if (showTools) {
    setApplicationMenu();
  }
