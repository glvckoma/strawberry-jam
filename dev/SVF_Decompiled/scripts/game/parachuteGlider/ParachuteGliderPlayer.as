package game.parachuteGlider
{
   ...
   public class ParachuteGliderPlayer
   {
      ...
      private static const PHANTOM_HOLD_TIME:Number = 0.25;
      ...
      private static const POLLUTETIMER_RETURN_TO_IDLE:Number = 2.25;
      
      private static const POSITION_UPDATE_TIME:Number = 0.25;
      
      public var _theGame:ParachuteGlider;
      ...
      public function ParachuteGliderPlayer(param1:ParachuteGlider)
      {
         super();
         _theGame = param1;
      ...
      public function init(param1:Array, param2:int, param3:Object, param4:Object) : int
      {
         _progressbar = param4;
         ...
         if(!_waitingForAllLoaded)
         {
            ...
            if(!_landed)
            {
               if(_landedReceived == 0)
               {
                  ...
                  {
                     for each(_loc21_ in _theGame._winds)
                     {
                        if(_loc21_.clone)
                        {
                           _loc18_ = _loc21_.clone.getBounds(_theGame._layerWinds);
                           ...
                           {
                              if(_loc21_.ySpeed >= 0 || _serverWindAcceleration.y != 0 || _serverPosition.y > _loc18_.y + 50)
                              {
                                 if(_serverWindAcceleration.x == 0 && _loc21_.xSpeed != 0 || _serverWindAcceleration.y == 0 && _loc21_.ySpeed != 0)
                                 {
                                    _loc10_ = true;
                                    if(_serverWindAcceleration.x == 0 && _loc21_.xSpeed != 0)
                                    {
                                       _serverWindTargetAcceleration.x = _loc21_.xSpeed;
                                    }
                                    if(_serverWindAcceleration.y == 0 && _loc21_.ySpeed != 0)
                                    {
                                       _serverWindTargetAcceleration.y = _loc21_.ySpeed;
                                    }
                  ...
                  {
                     _loc9_ = 0;
                     while(_loc9_ < _theGame._treeBranches.length)
                     {
                        if(_theGame._treeBranches[_loc9_].clone && _theGame._treeBranches[_loc9_].enabled && (_loc8_.y + _radius >= _theGame._treeBranches[_loc9_].clone.y && _loc8_.y <= _theGame._treeBranches[_loc9_].clone.y + _theGame._treeBranches[_loc9_].clone.height))
                        {
                           _loc19_ = _theGame.PlayerBranchCollision(_loc8_,_lastFrameVelocity,_radius,_theGame._treeBranches[_loc9_]);
                           if(_loc19_ >= 0)
                           {
                              if(_lastTreeBranchHit != _theGame._treeBranches[_loc9_])
                              {
                                 _lastTreeBranchHit = _theGame._treeBranches[_loc9_];
                                 if(_lastFrameVelocity.y > 0)
                                 {
                                    _serverAcceleration.y = -225;
                                    if(_serverVelocity.y > 0)
                                    {
                                       if(_localPlayer)
                                       {
                                          playAnimationState(24);
                                          _loc20_ = _loc9_;
                                          _theGame._treeBranches[_loc9_].clone.content.gotoAndPlay("on");
                                          if(_theGame._treeBranches[_loc9_].removeAfterHit)
                                          {
                                             _theGame._soundMan.playByName(_theGame._soundNameCloudSwirl);
                                             _theGame._treeBranches[_loc9_].enabled = false;
                                          }
                                          else
                                          {
                                             _theGame._soundMan.playByName(_theGame._soundNameTreeBranch);
                                          }
                                       }
                                       _serverVelocity.y = -50;
                                       _lastFrameVelocity.y *= _loc19_;
                                       _positionHoldTimer = 0.15;
                                       _serverWindAcceleration.x = 0;
                                       _serverWindTargetAcceleration.x = 0;
                                       _serverWindAcceleration.y = 0;
                                       _serverWindTargetAcceleration.y = 0;
                        ...
                        else if(_lastTreeBranchHit == _theGame._treeBranches[_loc9_])
                        {
                           _lastTreeBranchHit = null;
                        }
                        _loc9_++;
                     }
                  }
               ...
               if(_localPlayer)
               {
                  ...
                  if(_updatePositionTimer > 0.25 || _loc20_ != -1 || _loc22_ || _loc16_ || _loc10_)
                  {
                     _loc2_ = [];
                     _loc2_[0] = "pos";
                     ...
                     {
                        _loc2_[7] = "1";
                        _loc2_[8] = String(int(_theGame._treeBranches[_loc20_].branchID));
                     }
                     else
                     {
                        _loc2_[7] = "0";
      ...
      public function receivePositionData(param1:Array, param2:int) : int
      {
         ...
         loop2:
         switch(_serverHitType - 1)
         {
            case 0:
               _loc4_ = int(param1[param2++]);
               _positionHoldTimer = 0.15 / 2;
               playAnimationState(24);
               _loc6_ = 0;
               while(true)
               {
                  if(_loc6_ >= _theGame._treeBranches.length)
                  {
                     break loop2;
                  }
                  if(_loc4_ == _theGame._treeBranches[_loc6_].branchID)
                  {
                     if(_theGame._treeBranches[_loc6_].clone)
                     {
                        _theGame._treeBranches[_loc6_].clone.content.gotoAndPlay("on");
                     }
                     if(_theGame._treeBranches[_loc6_].removeAfterHit)
                     {
                        _theGame._treeBranches[_loc6_].enabled = false;
                     }
                     break loop2;
                  }
                  _loc6_++;
