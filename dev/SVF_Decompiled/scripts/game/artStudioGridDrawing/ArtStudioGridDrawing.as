package game.artStudioGridDrawing
{
   ...
   public class ArtStudioGridDrawing extends GameBase implements IMinigame
   {
      ...
      private var _gridPixelSize:int;
      
      private var _gridPencil:Object;
      ...
      public function ArtStudioGridDrawing()
      {
         super();
      }
      ...
      private function init() : void
      {
         ...
         _mediaLoader.init(5871,onMediaLoaded);
         try
         {
         ...
         if(!_bInit)
         {
            ...
            loadScene("ArtStudioGridDrawing/room_main.xroom");
            _bInit = true;
      ...
      private function moveMouse(param1:Event) : void
      {
         if(_sceneLoaded && _brush)
         {
            ...
            if(_drawing)
            {
               ...
               {
                  case 0:
                     _gridPencil._drawTranslateMatrix.tx = (_drawCanvas.mouseX - _scrollX) / _zoomFactor - (_drawCanvas.mouseX - _scrollX) / _zoomFactor % _gridPixelSize - _gridPencil._drawOffSet;
                     _gridPencil._drawTranslateMatrix.ty = (_drawCanvas.mouseY - _scrollY) / _zoomFactor - (_drawCanvas.mouseY - _scrollY) / _zoomFactor % _gridPixelSize - _gridPencil._drawOffSet;
                     _drawBitmapData.lock();
                     _drawBitmapData.draw(_gridPencil._drawBrushData,_gridPencil._drawTranslateMatrix,_brush.loader.content.brushTip.transform.colorTransform);
                     _drawBitmapData.unlock(new Rectangle(_gridPencil._drawTranslateMatrix.tx,_gridPencil._drawTranslateMatrix.ty,_gridPencil.diameter,_gridPencil.diameter));
               }
            }
         }
      }
      ...
      override protected function sceneLoaded(param1:Event) : void
      {
         ...
         var _loc4_:int = 0;
         var _loc6_:int = 0;
         _loc9_ = _scene.getLayer("closeButton");
         addBtn("CloseButton",_loc9_.x,_loc9_.y,onCloseButton);
         ...
         _gridPixelSize = _interfaceObject.loader.content.pixelSize;
         var _loc8_:Shape = new Shape();
         _loc8_.graphics.beginFill(2013265919,1);
         _loc8_.graphics.drawRect(0,0,_gridPixelSize,_gridPixelSize);
         _loc8_.graphics.endFill();
         _gridPencil = {};
         _gridPencil.diameter = _gridPixelSize;
         _gridPencil._drawOffSet = 0;
         _gridPencil._drawTranslateMatrix = new Matrix();
         _gridPencil._drawBrushData = new BitmapData(_loc8_.width,_loc8_.height,false,16711680);
         _gridPencil._drawBrushData.draw(_loc8_,null);
         ...
         _layerGrid.visible = false;
         _layerGrid.mouseEnabled = false;
         _layerGrid.mouseChildren = false;
         var _loc3_:BitmapData = new BitmapData(_interfaceObject.loader.content.canvas.width,_interfaceObject.loader.content.canvas.height,true,16777215);
         var _loc7_:Bitmap = new Bitmap(_loc3_,"auto",false);
         _layerGrid.addChild(_loc7_);
         _layerGrid.x = 0;
         _layerGrid.y = 0;
         _loc8_ = new Shape();
         _loc8_.graphics.beginFill(0,0.1);
         _loc8_.graphics.lineStyle(1,0,0.1,true);
         _loc4_ = _gridPixelSize;
         while(_loc4_ < _interfaceObject.loader.content.canvas.width)
         {
            _loc8_.graphics.moveTo(_loc4_ - 1,0);
            _loc8_.graphics.lineTo(_loc4_ - 1,_interfaceObject.loader.content.canvas.height);
            _loc4_ += _gridPixelSize;
         }
         _loc6_ = _gridPixelSize;
         while(_loc6_ < _interfaceObject.loader.content.canvas.height)
         {
            _loc8_.graphics.moveTo(0,_loc6_ - 1);
            _loc8_.graphics.lineTo(_interfaceObject.loader.content.canvas.width,_loc6_ - 1);
            _loc6_ += _gridPixelSize;
         }
         _loc8_.graphics.endFill();
         _loc3_.draw(_loc8_,null);
         ...
         _interfaceObject.loader.content.canvas.addChild(_layerGrid);
         _undoable = false;
      ...
      private function clearOutItemLayer(param1:MovieClip) : void
      {
         ...
         if(param1)
         {
            while(param1.itemLayer.numChildren > 0)
            {
               ...
               if(param1.itemLayer.numChildren > 0)
               {
      ...
      private function setupCurrIconAndBitmap(param1:int) : void
      {
         ...
         if(_masterpiecePreviewIcons[param1 + _itemOffset])
         {
            ...
            _loc6_.newTag.visible = param1 == 0 && _itemOffset == 0;
      ...
      private function onSubmissionRulesClose(param1:Boolean, param2:Boolean) : void
      {
         ...
         {
            setupRequest(null,null,20,0,_isUsingTokens,1,false);
         }
      ...
      private function onSaveButton() : void
      {
         ...
         try
         {
            ...
            _loc1_.save(_loc2_,"painting.ajgart");
         }
         catch(e:Error)
         {
         }
      ...
      private function onLoadButton() : void
      {
         ...
         try
         {
            ...
            _loc2_ = [];
            _loc1_ = new FileFilter("AJGArt Files (*.ajgart)","*.*");
            _loc2_.push(_loc1_);
