...
const TARGET_ASAR = path.join(__dirname, 'assets', 'winapp.asar');
...
function runElectron(chalk) {
    ...
    try {
        ...
        console.log(chalk.gray(`Running script: ${indexPath}`));
...
async function main() {
    try {
        ...
        const answers = await prompt([
            {
                ...
                choices: [
                    ...
                    { name: 'Development (from assets/extracted-winapp-dev)', value: 'dev' }, 
        ...
        let sourceDir;
        if (answers.versionChoice === 'dev') {
            sourceDir = DEV_SOURCE_DIR;
        } else if (answers.versionChoice === 'public') {
